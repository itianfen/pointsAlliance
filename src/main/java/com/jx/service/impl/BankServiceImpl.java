package com.jx.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.jx.dao.BankDao;
import com.jx.entity.Bank;
import com.jx.entity.Card;
import com.jx.entity.Token;
import com.jx.service.BankService;
import com.jx.service.CardService;
import com.utils.*;
import org.springframework.stereotype.Service;

import javax.annotation.Resource;
import java.util.*;

/**
 * (Bank)表服务实现类
 *
 * @author makejava
 * @since 2020-07-14 17:07:26
 */
@Service
public class BankServiceImpl extends ServiceImpl<BankDao, Bank> implements BankService {
    @Resource
    private BankDao bankDao;
    @Resource
    private CardService cardService;

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    @Override
    public Map<String, Object> queryById(String id) {
        if (StringUtils.isEmpty(id)) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        Bank bank = this.bankDao.queryById(id);
        if (bank == null) {
            return Response.fail(ErrorItem.NO_RESULT_ERROR);
        }
        return Response.success(bank);
    }

    /**
     * 查询多条数据
     *
     * @param bank           实体对象条件类
     * @param page           页数
     * @param limit          每页条数
     * @param preciseOrFuzzy 精确 1/模糊 0
     * @param timeArea       时间范围 以 ~ 分隔
     * @return 对象列表
     */
    @Override
    public Map<String, Object> queryAllByLimit(Bank bank, Integer page, Integer limit, Integer preciseOrFuzzy, String timeArea) {
        if (page == null || limit == null) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        if (preciseOrFuzzy == null) {
            preciseOrFuzzy = 1;
        }
        if (page < 1) {
            page = 1;
        }
        long offset = (page - 1) * limit;
        //时间范围
        Date begin = null, end = null;
        if (StringUtils.isNotEmpty(timeArea)) {
            String[] times = timeArea.split("~");
            begin = DateFormatUtils.parseDateString(times[0], DateFormatUtils.defaultFormat);
            end = DateFormatUtils.parseDateString(times[1], DateFormatUtils.defaultFormat);
        }
        if (bank == null) {
            bank = new Bank();
        }
        bank.setDeleted(0);
        //列表
        List<Bank> list = this.bankDao.queryAllByLimit(bank, offset, limit, preciseOrFuzzy, begin, end);
        //总条数
        long count = this.bankDao.countByParam(bank, preciseOrFuzzy, begin, end);
        return Response.success(list, count);
    }

    /**
     * 新增数据
     *
     * @param bank 实例对象
     * @return 实例对象
     */
    @Override
    public Map<String, Object> insert(String token, Bank bank) {
        if (StringUtils.isEmpty(token)) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        Token tokenObj = TokenUtils.getTokenObj(token);
        if (tokenObj == null) {
            return Response.fail(ErrorItem.AUTH_TRANSCEND_ERROR);
        }
        Date now = DateFormatUtils.getCurrentDate();
        bank.setId(UUIDUtils.generate());
        bank.setCreatedTime(now);
        bank.setLastUpdateTime(now);
        bank.setCreator(tokenObj.getUserName());
        bank.setCreatorId(tokenObj.getUserId());
        bank.setLastUpdater(tokenObj.getUserName());
        bank.setLastUpdateId(tokenObj.getUserId());
        bank.setDeleted(0);
        if (this.bankDao.insert(bank) > 0) {
            return Response.success(bank);
        }
        return Response.fail(ErrorItem.INSERT_ERROR);
    }

    /**
     * 修改数据
     *
     * @param bank 实例对象
     * @return 实例对象
     */
    @Override
    public Map<String, Object> update(String token, Bank bank) {
        if (StringUtils.isEmpty(token, bank.getId())) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        Token tokenObj = TokenUtils.getTokenObj(token);
        if (tokenObj == null) {
            return Response.fail(ErrorItem.AUTH_TRANSCEND_ERROR);
        }
        bank.setLastUpdateTime(DateFormatUtils.getCurrentDate());
        bank.setLastUpdater(tokenObj.getUserName());
        bank.setLastUpdateId(tokenObj.getUserId());
        bank.setDeleted(0);
        return this.bankDao.update(bank) > 0 ? Response.success() : Response.fail(ErrorItem.UPDATE_ERROR);
    }

    /**
     * 修改数据（全部字段）
     *
     * @param bank 实例对象
     * @return 实例对象
     */
    @Override
    public Map<String, Object> updateAllColumn(String token, Bank bank) {
        if (StringUtils.isEmpty(token, bank.getId())) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        Token tokenObj = TokenUtils.getTokenObj(token);
        if (tokenObj == null) {
            return Response.fail(ErrorItem.AUTH_TRANSCEND_ERROR);
        }
        bank.setLastUpdateTime(DateFormatUtils.getCurrentDate());
        bank.setLastUpdater(tokenObj.getUserName());
        bank.setLastUpdateId(tokenObj.getUserId());
        bank.setDeleted(0);
        return this.bankDao.update(bank) > 0 ? Response.success() : Response.fail(ErrorItem.UPDATE_ERROR);
    }

    /**
     * 通过主键删除数据
     *
     * @param id 主键列表 字符串根据,分隔
     * @return 是否成功
     */
    @Override
    public Map<String, Object> deleteById(String token, String id) {
        if (StringUtils.isEmpty(token, id)) {
            return Response.fail(ErrorItem.MAST_PARAM_NOT_SET);
        }
        Token tokenObj = TokenUtils.getTokenObj(token);
        if (tokenObj == null) {
            return Response.fail(ErrorItem.AUTH_TRANSCEND_ERROR);
        }
        //转为换维数组格式
        String[] idArr = id.split(",");
        List<String> idList = new ArrayList<>();
        Collections.addAll(idList, idArr);
        //判断银行下有没有商品
        List<Card> list = cardService.list(new QueryWrapper<Card>().eq(Card.COL_BANK_ID, id).eq(Bank.COL_DELETED, 0));
        if (!list.isEmpty()) {
            return Response.fail(ErrorItem.DELETE_CARD_FIRST);
        }

        //逻辑删除
        if (this.bankDao.deleteByIdNotTrue(idList, DateFormatUtils.getCurrentDate(), tokenObj.getUserName(), tokenObj.getUserId()) > 0) {
            return Response.success();
        }
        return Response.fail(ErrorItem.DELETE_ERROR);
    }

}

